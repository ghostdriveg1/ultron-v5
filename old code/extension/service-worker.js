...
async function getOrCreateProviderTab(providerKey) {
  ...
  if (matchedTab) {
    ...
    await storage.appendLog('info', `Reusing existing tab for ${adapter.name}...`);
    // Make the tab active so the user can see it!
...
function waitTabLoaded(tabId) {
  return new Promise((resolve) => {
    let timeoutId = setTimeout(() => {
      ...
      storage.appendLog('warn', `Tab load timeout for tab ${tabId}. Proceeding anyway.`);
      resolve();
    }, 15000);
